...
function calculateIpdBill () {
    ...
    bedCharge = parseInt(removeCommas($('#bedCharge').text()))
    ...
    netPayabelAmount = (totalCharges + otherCharges + totalTax + bedCharge) -
        (totalPayments + totalDiscount)
    if (netPayabelAmount > 0)
        $('#billStatus').html(Lang.get('messages.employee_payroll.unpaid'))
    else {
        netPayabelAmount = 0
