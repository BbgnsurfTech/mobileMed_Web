...
function loadIpdPatientCreate () {
    ...
    $('#ipdCaseId, #editIpdCaseId ').select2({
        width: '100%',
        placeholder: Lang.get('messages.ipd_patient.choose_case'),
    })

    $('#ipdBedId, #editIpdBedId').select2({
        width: '100%',
        placeholder: Lang.get('messages.bed.choose_bed'),
    })

    let admissionFlatPicker = $('#ipdAdmissionDate, #editIpdAdmissionDate').
        flatpickr({
            enableTime: true,
            ...
            success: function (data) {
                if (data.data !== null) {
                    if (data.data.length !== 0) {
                        ...
                        if (typeof $('#editIpdPatientBedId').val() !=
                            'undefined'
                            && typeof $('#editIpdPatientBedTypeId').val() !=
                            'undefined') {
                            if (bedType ===
